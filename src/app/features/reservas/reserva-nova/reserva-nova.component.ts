...
export class ReservaNovaComponent extends BaseFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Inicializa com os tipos obrigatórios na principal
    this.profissionaisPrincipal = this.criarProfissionaisObrigatorios();
  }

  ngOnDestroy(): void {
    // Libera o bloqueio ao sair da tela (navegação, voltar, etc)
        ...
        if (status.bloqueado && !status.bloqueadoPorOutroUsuario) {
          ...
          if (!this.unidadeBloqueada) {
            ...
            this.messageService.add({
              ...
              detail: 'O tempo para completar a reserva expirou.',
              life: 4000
            });
    ...
    this.reservaBloqueioService.bloquear({
      ...
      next: (response) => {
        ...
        this.messageService.add({
          ...
          detail: `Você tem ${this.formatarTempo(response.tempoRestanteSegundos)} para completar a reserva.`,
          life: 4000
        });
      },
      error: (error) => {
  ...
  private alertarTempoCritico(): void {
    this.messageService.add({
      ...
      detail: 'Restam apenas 30 segundos para completar a reserva!',
      life: 5000,
      sticky: false
    });
  }
  ...
  private criarProfissionaisObrigatorios(): ProfissionalForm[] {
    return this.getTiposProfissionaisObrigatorios().map(tipo => this.criarProfissionalForm(tipo, true));
  }
